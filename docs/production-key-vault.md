...
| `OPENAI_API_KEY` | `zeroops-ai-api-key` | Only if AI analysis is enabled | Optional analysis provider key |
...
These are configuration values, not Key Vault secrets: `APP_ENV=production`, `FRONTEND_URL`, `CORS_ORIGINS`, `ALLOWED_HOSTS`, `ZEROOPS_BACKEND_URL`, `AZURE_KEYVAULT_URL`, `AZURE_DEFAULT_REGION`, `MAX_CODE_UPLOAD_MB`, and `DB_SSL_VERIFY=true`.
