...
The frontend and backend deployment jobs use the protected GitHub environment
named `production`. GitHub therefore issues an OIDC token with this exact
subject:
...
The client IDs come from the GitHub `production` environment secrets selected
by `.github/workflows/main_zeroopsai.yml` and
...
If a credential named `github-production` already exists, inspect its issuer,
