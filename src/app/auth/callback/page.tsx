...
function CallbackContent() {
  ...
  useEffect(() => {
    ...
    if (error) {
      ...
      setErrorMessage(messages[error] || `Authentication failed: ${error}`);
      return;
    }

    if (!token) {
      setStatus("error");
      setErrorMessage("No token provided in callback request.");
      return;
    }
    ...
    try {
      ...
      // Also write cookie so standard fetch gets it
      const isLocal = window.location.hostname === "localhost" || window.location.hostname === "127.0.0.1";
      document.cookie = `session_token=${token}; path=/; max-age=2592000; SameSite=Lax${isLocal ? "" : "; Secure"}`;
      ...
      refreshUser()
        .then(() => {
          ...
          // Let's call the API to fetch repositories so they are cached / verified
          return api.getGitHubRepos({ page: 1, per_page: 10 });
        })
        .then(() => {
        ...
        .catch((err) => {
          ...
          // If repository fetch failed but user is authenticated, we still succeed and proceed to repo page
          setStep(3);
          setStatus("success");
          setTimeout(() => {
            router.push("/dashboard/repositories");
